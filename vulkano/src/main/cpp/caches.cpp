...
bool FramebufferAllocation::matches(const VkFramebufferCreateInfo &info) const {
  return pass == info.renderPass && width == info.width &&
         height == info.height && layers == info.layers &&
         views.size() == info.attachmentCount &&
         std::equal(views.begin(), views.end(), info.pAttachments);
}

DescriptorPoolAllocation
Device::takeDescriptorPool(uint32_t maxSets,
                           const std::vector<VkDescriptorPoolSize> &sizes) {
  for (size_t i = 0; i < idleDescriptorPoolCount; ++i) {
    const auto &candidate = idleDescriptorPools[i];
    if (candidate.maxSets != maxSets ||
        candidate.sizes.size() != sizes.size() ||
        !std::equal(sizes.begin(), sizes.end(), candidate.sizes.begin(),
    ...
    idleDescriptorPools[i] =
        std::move(idleDescriptorPools[--idleDescriptorPoolCount]);
  ...
  check(vkCreateDescriptorPool(device, &info, nullptr, &allocation.pool),
...
Device::takeFramebuffer(const VkFramebufferCreateInfo &info) {
  for (size_t i = 0; i < idleFramebufferCount; ++i) {
    if (!idleFramebuffers[i].matches(info))
      continue;
  ...
  FramebufferAllocation allocation{
      VK_NULL_HANDLE,
      info.renderPass,
      std::vector<VkImageView>(info.pAttachments,
                               info.pAttachments + info.attachmentCount),
      info.width,
      info.height,
      info.layers};
  check(vkCreateFramebuffer(device, &info, nullptr, &allocation.framebuffer),
        "vkCreateFramebuffer");
  ++framebuffersCreated;
  return allocation;
}

void Device::recycle(DescriptorPoolAllocation allocation,
                     bool completed) noexcept {
  uint64_t descriptors = 0;
  for (auto size : allocation.sizes)
    descriptors += size.descriptorCount;
  if (completed && descriptors <= 4096 &&
      idleDescriptorPoolCount < idleDescriptorPools.size() &&
