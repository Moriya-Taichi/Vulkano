...
class FrameScheduler internal constructor(
    ...
    fun beginFrame(): Frame? = device.access {
    ...
    internal fun submit(frame: Frame): Unit = device.access {
        check(!closed && active === frame) { "Frame is no longer active" }
        try { frame.commandBuffer.commit() }
        finally { release(frame) }
    }

    private fun release(frame: Frame) {
        if (frame.commandBuffer.isClosed) { active = null; frame.finished = true; return }
    ...
    internal fun abandon(frame: Frame) = synchronized(device) {
        if (!frame.finished && active === frame) {
            if (device.closed) { frame.commandBuffer.close(); active = null; frame.finished = true }
            else release(frame)
        }
    }

    override fun close() {
        val commands = synchronized(device) {
            if (closed) return
            closed = true
        ...
        for (command in commands) try { command.close() } catch (error: Throwable) { if (failure == null) failure = error }
...
fun Device.makeFrameScheduler(maxFramesInFlight: Int = 3, queueIndex: Int = 0): FrameScheduler = access {
