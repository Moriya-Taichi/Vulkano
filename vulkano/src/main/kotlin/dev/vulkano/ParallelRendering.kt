...
internal constructor(command: CommandBuffer, private var nativeEncoder: Long) :
    ...
    fun makeRenderCommandEncoder(): RenderCommandEncoder = encode {
        RenderCommandEncoder(commandBuffer, Native.beginParallelRenderChild(nativeEncoder)).also {
            children.add(it)
        }
    }
    ...
    internal fun childEnded(child: CommandEncoder) {
        check(children.remove(child))
    }

    override fun finish() {
        check(children.isEmpty()) {
            "End every child render encoder before ending parallel recording"
        }
