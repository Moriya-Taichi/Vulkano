...
Attachmentの読み取りはSubpassとInput Attachmentで指定でき、BY_REGIONの依存関係を入れます。Attachmentと同じTextureを同一Pass内でSamplingするFeedback Loopは禁止します。Textureは必要に応じてTransfer、General、Attachment、PresentationのLayoutへ移行します。
